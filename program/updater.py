...
@Client.on_message(command(["مبرمج السورس", f"update@{BOT_USERNAME}"]) & ~filters.edited)
@sudo_users_only
async def update_repo(_, message: Message):
    chat_id = message.chat.id
    msg = await message.reply("🔄 `برمجه وكتابه الـ ميم . ...`")
    update_avail = updater()
    if update_avail:
        await msg.edit("👤 يمكن شراء بوت بحقوقك كامل \n\n• او نسخه سورس بحقوقك بأرخص الأسعار حسابي الوحيد : N_J_9.")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit("My only **account on ** telegram [X : Mem •](https://t.me/N_J_9)", disable_web_page_preview=True)
